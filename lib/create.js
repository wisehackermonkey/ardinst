...
let prefix = childProcess.execSync('npm root -g').toString().trim();
...
exports.symlink = (library_name) => {
    let arduino_folder_path = path.join(os.homedir(), "Documents", "Arduino", "libraries");

    let arduino_folder = path.join(arduino_folder_path, library_name);
    let node_folder = path.join(prefix, library_name);
...
exports.publish = () => {
  return `___________TODO_____________`;
};
